...
> ⚠️ **本条曾经写反，已于 2026-08-30 订正（原文见下）。`PROTON_VERB=run` 必须设，现已加回实现。**
>
> 原文声称"参考脚本从未设置这个变量，umu-run 的默认行为已经是「运行游戏」"。**两句都错**：
> `scripts/ark_instance_manager.sh` 在 `start_server()` 的**第一行**（L884）就 `export PROTON_VERB=run`，
> 靠环境继承传给 L992 那条启动命令；当初对拍时只看了启动命令行本身，漏了函数开头的 export。
> 而 umu 的默认动词是 `waitforexitandrun`，它会先跑 `wineserver -w` 等该 prefix 的现有游戏退出——
> 共享 prefix 下第二个实例因此永远卡住，游戏进程从不被 exec，上层只看到
> 「游戏进程在 3m0s 内没有出现」。2026-08-30 真机确认。
> 详见 `docs/UMU_PREFIX_PER_INSTANCE_PLAN.md`。
>
> 教训：对拍 shell 脚本时，"这条命令行上写了哪些变量"和"这个进程实际继承了哪些变量"是两回事，
> 应以**函数整体**或运行中进程的 `/proc/<pid>/environ` 为准。
...
| 6 | **共享 prefix 的 wineserver 竞争** | 多实例并发首次启动可能互相踩；一个实例崩溃可能波及同 prefix 的其他实例 | 默认共享（与脚本一致，磁盘友好），但**首次初始化加互斥锁串行化**；`config.yaml` 提供 `prefix_mode: per-instance` 开关给追求隔离的用户。**🆕 2026-08-30**：这条风险已在真机上爆发，但实际机制不是"竞争"而是 Proton 默认动词 `waitforexitandrun` 造成的**永久排队**（根因是缺 `PROTON_VERB=run`，已修）；同时补了共享模式下的启动闸门。另需注意 `per-instance` 至今是**接线未完成的死配置**（无人传 `Options.PrefixKey`）。全部细节见 `docs/UMU_PREFIX_PER_INSTANCE_PLAN.md` |
...
> 以 `docs/APPCONFIG_BASEDIR_PLAN.md` 为准**，那份文档已实施完毕。**G0–G5 全部已实施**
> （落地记录见 G1 真 bug 说明之后的"G3/G4 落地记录"）；G4 的 umu/GE-Proton/SteamCMD
> 流程仍缺真实 Linux 主机的端到端跑通，见该记录最后一条。**
...
> **G3/G4 落地记录**：
>
> - §10.4 的两条校验规则实现为 `appconfig.ValidateBaseDir(dir) error`（`internal/appconfig/wizard.go`），
>   Fyne 对话框回调与 `asa-server setup` 都调它，没有各写一份。剩余空间检查用
>   `github.com/shirou/gopsutil/v4/disk`（项目已经依赖这个模块，免费拿到跨平台实现，见新增的
>   `pkg/fsutil/diskspace.go`）；网络盘检测是新写的 `pkg/fsutil/drivetype_{windows,linux}.go`——
>   Windows 用 `golang.org/x/sys/windows.GetDriveType`（已知缺口：不识别 `subst` 虚拟盘，需要额外
>   `QueryDosDevice` 才能覆盖，本次未做），Linux 解析 `/proc/mounts` 按最长前缀匹配挂载点。
> - "全新安装 vs 老部署（不弹窗）"这条判断没有再传一个新的目录参数或新的返回值——`appconfig.Load()`
>   内部本来就知道这次是不是靠三级查找兜底生成的默认模板，新增一个包级原子标志
>   `appconfig.WasConfigAutoGenerated()` 暴露这个既有事实，`Load()` 的签名不用再变。
> - 写回用户选择的目录用 `appconfig.WriteInitialConfig(baseDir)`：不是重新序列化一份新文件，而是在
>   G1 自动生成的默认模板里精确查找 `basedir: ""` 这一行做原地替换——保留模板给用户看的大段注释；
>   字段已经不是这个占位符时（比如函数被误调用在已配置过的文件上）直接拒绝，不做任何改写。
> - Fyne 端（`internal/gui/gui.go`）：向导写完配置后不需要重启进程，`applyChosenBaseDir` 原地重放一遍
>   `appconfig.Load()` → `cfgpkg.EnsureDirectories` → `logger.InitLoggerWithBaseDir` →
>   `runner.Configure`，和 `main.go` 启动时的顺序一致。
> - `asa-server setup`（`internal/actions/setup.go`）没有拆 `_windows.go`/`_linux.go`——它调用的
>   `runner`/`installer` 本身两平台都能编译，只有一处 `runtime.GOOS != "linux"` 的早退分支，
>   不需要构建约束隔离。
> - **诚实说明测试覆盖的边界**：G3 在本机（Windows）做了真实编译产物的冒烟测试——全新目录下启动
>   GUI，确认新增的向导触发代码路径不会导致启动崩溃（进程存活、主窗口正常创建）；但没有可用的 UI
>   自动化工具真正点击文件夹选择对话框走完整条路径，那部分是代码审查覆盖，不是端到端跑过的。
>   G4 做了跨平台交叉编译（`GOOS=linux go build`/`go vet` 全过）和逻辑审查，但没有真实 Linux
>   主机跑一遍 umu/GE-Proton/SteamCMD 的下载安装——这部分本来就依赖 `docs/LINUX_COMPATIBILITY_PLAN.md`
>   开头说的"真实 Linux 主机上的端到端验证仍待补"，不是这次新引入的缺口。
...
| 能力 | Windows | Linux |
|---|---|---|
| 执行 ARK exe | `exec.Command(exe, args...)` | `umu-run <exe> <args...>`（env: `WINEPREFIX`/`GAMEID`/`PROTONPATH`/`UMU_RUNTIME_UPDATE=0`/**`PROTON_VERB=run`**，见 §5.1） |

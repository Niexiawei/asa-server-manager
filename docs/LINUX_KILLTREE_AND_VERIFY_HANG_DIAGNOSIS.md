...
> 状态：**Q1 / Q1b / Q2 / Q3 / Q4 / Q6 已确认，全部修复已落地（见 §7 实施记录）**。
> Q2 挖出第四个更严重的问题（§2.5）；Q4 证明权限缺口不止 `Saved` 一处，
> 最终采用方案 B（组 + setgid + 默认 ACL）+ A（chown）兜底（§3.7）；
> Q6 证明 `waitForWineserverDrain` 一直是空转。
> **Q5 / Q7 待在真机上复测**（§7.4 列了验证步骤）。
...
| Q5 | 端口监听了吗？ | 见下 | ⚠️ **仍未监听**（与 Q4 的卡点一致，等 §3.6 修完再复测） |
| Q6 | `waitForWineserverDrain` 是不是空转？ | 见下 | ✅ **是空转**，见下方实测 |
| Q7 | CFCore 的 curseforge/overwolf 能连通吗？ | 见 §3.6 | ⬜ 待确认（仅当 §3.6 修完仍卡才需要） |
...
1. `asa-server verify` —— 看 `{BaseDir}/logs/verify-launch.log` 与
   `Saved/Logs/ShooterGame.log` 是否越过了 CFCore 那几行、端口是否监听（**Q5**）
2. 若仍卡在 CFCore，跑 §3.6 的 curl 探测（**Q7**）
3. 起一个实例，确认 `asaServer.log` 里**不再**出现
   `ArkAscendedServer.exe did not appear within 30 seconds`
4. 停止实例，确认 `pgrep -af 'Z:.*ArkAscendedServer\.exe'` 为空（§2.4 生效）

...
// runFirstLaunchWizardIfNeeded 是 docs/LINUX_COMPATIBILITY_PLAN.md §10.4/§10.5 G3
// 的入口：只有 appconfig.WasConfigAutoGenerated() 为 true（main.go 启动时那次
// appconfig.Load() 三级查找哪儿都没找到、纯靠兜底生成了默认模板）才是真正的全新
// 安装，需要弹目录选择对话框；任一级已有旧版 config.yaml（哪怕没有 basedir 字段）
// 都维持现状不弹窗——这条判断本身在 appconfig.Load() 里已经做完了，这里只是读结果。
//
// 必须在 g.window 已经创建（createMainWindow 之后）才能调用：Fyne 的对话框需要一
// 个父窗口。
...
// showBaseDirPicker 弹目录选择对话框，校验通过后写回 config.yaml 并原地刷新
// BaseDir（不需要重启程序，见 G3 的验收判据）。校验失败会把原因显示出来并重新
// 弹一次；用户直接取消对话框（不选目录）时保留"不设置、直接用 exe 目录跑"这条
// 退路，不强制——§10.4 明确要求这条退路必须在。
func (g *GUIApp) showBaseDirPicker() {
	fd := dialog.NewFolderOpen(func(uri fyne.ListableURI, err error) {
		if err != nil {
			g.showError(fmt.Errorf("选择数据目录失败: %w", err))
			return
		}
		if uri == nil {
			// 用户点了取消：保留退路，继续用 exe 同级目录这个默认值，不强制。
			return
		}
		chosen := uri.Path()
		if verr := appconfig.ValidateBaseDir(chosen); verr != nil {
			g.showError(verr)
			g.showBaseDirPicker()
			return
		}
		if werr := appconfig.WriteInitialConfig(chosen); werr != nil {
...
func (g *GUIApp) Run() {
	...
	// 全新安装时弹数据目录选择向导（G3，见 runFirstLaunchWizardIfNeeded 的文档）；
	// 必须放在 createMainWindow 之后——对话框需要 g.window 已经存在。
	g.runFirstLaunchWizardIfNeeded()

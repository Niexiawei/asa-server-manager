...
// PrefixInfo is one Wine prefix directory found on disk. Alias of
// pkg/wineprefix.Info — see docs/RUNNER_INSTANCE_PACKAGE_SPLIT_PLAN.md 阶段 I.
//
// runner deliberately does NOT know what an "instance" is, so it can't say
// whether a prefix is an orphan — it only reports the key it was created
// for. Cross-referencing keys against real instances is the caller's job
// (see internal/actions/prefix.go).
type PrefixInfo = wineprefix.Info

...
// PerLevelStatsMultiplier 的分组。注意 _Add / _Affinity 必须排在 DinoTamed 之前匹配。
export const STAT_GROUPS = [
  { key: 'Player', label: '玩家' },
  { key: 'DinoWild', label: '野生生物' },
  { key: 'DinoTamed', label: '驯养生物' },
  { key: 'DinoTamed_Add', label: '驯服加成 (Add)' },
  { key: 'DinoTamed_Affinity', label: '驯服亲和 (Affinity)' },
]
...
const STAT_LINE_RE = /^PerLevelStatsMultiplier_(DinoTamed_Add|DinoTamed_Affinity|DinoTamed|DinoWild|Player)\[(\d+)\]$/
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    try {
      if (CLASS_MULTIPLIER_KEY_SET.has(key)) {
        ...
        const m = key.match(STAT_LINE_RE)
        if (m && model.stats[m[1]]) {
          model.stats[m[1]][Number(m[2])] = toNum(value)
          absorbed.add(i)
        }
      }
    } catch {
...
function serializeManaged(model) {
  ...
  for (const { key } of STAT_GROUPS) {
    ...
    for (let i = 0; i < STAT_INDICES.length; i++) {
      const v = group[i]
      if (v !== '' && v != null) out.push(`PerLevelStatsMultiplier_${key}[${i}]=${fmtNum(v)}`)
    }
  }

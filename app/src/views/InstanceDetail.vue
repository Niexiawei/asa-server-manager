        ...
        <t-collapse v-model:value="activeCollapseKeys" class="config-files-collapse">
          ...
          <t-collapse-panel value="backup-list" header="游戏存档备份">
            ...
            <t-loading :loading="backupListLoading">
              <div
                  v-if="backups.length === 0 && !backupListLoading"
                  style="color: #999; padding: 16px 0"
              >
                暂无备份
              </div>
              <t-row :gutter="[12, 12]">
                <t-col :span="4" v-for="item in backups" :key="item">
                  <t-card size="small" :bordered="true">
                    ...
                    <div style="font-size:12px; word-break:break-all; color:#555; margin-bottom:8px">{{ item }}</div>
                    <t-space size="small">
                      <t-button size="small" theme="primary" @click="restoreBackupHandler(item)">恢复</t-button>
                      <t-button size="small" theme="danger" variant="outline" @click="deleteBackupHandler(item)">删除
                      </t-button>
                    </t-space>
                  </t-card>
                </t-col>
